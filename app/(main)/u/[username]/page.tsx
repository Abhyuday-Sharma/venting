...
import { getUserProfileByUsername, getPublicVentsByUserId } from "@/lib/firebase";
...
export default function UserProfilePage() {
    ...
    useEffect(() => {
        if (username) {
            setLoading(true);
             getUserProfileByUsername(username).then(userProfile => {
                if (userProfile) {
                    setProfile(userProfile);
                    // Fetch vents using the retrieved profile's UID
                    getPublicVentsByUserId(userProfile.uid).then(userVents => {
                        setVents(userVents);
                        setLoading(false);
                    });
                } else {
