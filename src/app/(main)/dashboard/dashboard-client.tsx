...
export function DashboardClient() {
  ...
  useEffect(() => {
    try {
      const trigger = sessionStorage.getItem('acknowledgementTrigger');
      if (trigger === 'true') {
  ...
  let greeting = "";
  if (hour < 12) {
    greeting = `Good morning, ${user?.username || 'Friend'}. How are you feeling today?`;
  } else if (hour < 17) {
    greeting = `Good afternoon, ${user?.username || 'Friend'}. Take a moment for yourself.`;
  } else {
    greeting = `Good evening, ${user?.username || 'Friend'}. Let the day go.`;
  }

  return (
    <>
      <div className="relative container mx-auto p-4 md:p-8 space-y-8 overflow-hidden">
        {/* Ambient glow orbs */}
        <div className="ambient-orb w-80 h-80 bg-indigo-300 dark:bg-indigo-700 -top-24 -right-16 -z-10" />
        <div className="ambient-orb w-64 h-64 bg-pink-200 dark:bg-purple-900 bottom-8 left-0 -z-10" />

        {vents.length === 0 ? (
          <div className="glass-card card-reveal rounded-2xl text-center py-16 px-4">
            <h2 className="text-2xl font-semibold mb-2 font-headline">{greeting}</h2>
